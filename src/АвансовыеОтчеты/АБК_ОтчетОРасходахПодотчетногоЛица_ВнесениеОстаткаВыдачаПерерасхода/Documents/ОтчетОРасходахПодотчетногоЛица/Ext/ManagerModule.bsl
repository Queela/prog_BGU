...
Процедура АБК_ДобавитьКомандыПечати(КомандыПечати)
	...
	КомандаПечати.Представление = НСтр("ru = 'Отчет о расходах подотчетного лица (доработанный)'");
...
Процедура АБК_Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода)
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_ОтчетОРасходахПодотчетногоЛица504520_ОстатокПерерасход") Тогда
		...
		ПечатьОтчетаОРасходахПодотчетногоЛица_ВыдачаОстаткаПерерасхода(МассивОбъектов, ОбъектыПечати));

	КонецЕсли;

КонецПроцедуры     

Функция ПечатьОтчетаОРасходахПодотчетногоЛица_ВыдачаОстаткаПерерасхода(МассивОбъектов, ОбъектыПечати) Экспорт
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.ИмяПараметровПечати = "ПФ_MXL_ОтчетОРасходахПодотчетногоЛица504520_ОстатокПерерасход";
	
	//Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ОтчетОРасходахПодотчетногоЛица.ПФ_MXL_ОтчетОРасходахПодотчетногоЛица0504520"); 
	Макет = ПолучитьМакет("ПФ_MXL_ОтчетОРасходахПодотчетногоЛица504520_ОстатокПерерасход");
	...
	Пока Выборка.Следующий() Цикл
		...
		//Апогей
		 ...
		 //ТаблицаВыдачи = ПолучитьТЧВыдачиОстаткаПерерасхода(Выборка.Ссылка, Выборка.Сотрудник.Контрагент);
		 
		 ВывестиДопДанные(выборка.Ссылка, Выборка.Сотрудник.Контрагент, Макет, ТабличныйДокумент, СтруктураДанных);
		...
		Иначе
			...
			Пока СтрокиДО.Следующий() Цикл   
				...
				ТЧ = ПолучитьВыданныйОстатокПерерасход(Выборка.Ссылка);
				
				ТЧОтбор = ТЧ.НайтиСТроки(Новый Структура("КПС, КЭК", СтрокиДО.КБК, СтрокиДо.КОСГУ));
				СуммаОстатокПерерасход = 0;
				Если ТЧОтбор.Количество() Тогда 
					СуммаОстатокПерерасход = ТЧОтбор[0].Сумма;
				КонецЕсли;
				...
				ОбластьСтрокаДО.Параметры.Отклонение = СтрокиДО.Отклонение + СуммаОстатокПерерасход;
				...
				ИтогоОтклонение = ИтогоОтклонение + СтрокиДО.Отклонение + СуммаОстатокПерерасход;
				...
				НоваяСтрока.Отклонение = СтрокиДО.Отклонение + СуммаОстатокПерерасход;
...
Функция ПолучитьТЧВыдачиОстаткаПерерасхода(Ссылка, Контрагент) 
	
	ТабОстатокПерерасход = Новый ТаблицаЗначений;
	ТабОстатокПерерасход.Колонки.Добавить("ОстатокПерерасходНомерСчета", 			Новый ОписаниеТипов("Строка"));
	ТабОстатокПерерасход.Колонки.Добавить("ВнесенОстатокВРублях", 					Новый ОписаниеТипов("Число"));
	ТабОстатокПерерасход.Колонки.Добавить("ВнесенОстатокВВалюте", 					Новый ОписаниеТипов("Число"));
	ТабОстатокПерерасход.Колонки.Добавить("ВнесенОстатокВРубляхСПокрытиемБух", 		Новый ОписаниеТипов("Число"));
	ТабОстатокПерерасход.Колонки.Добавить("ВнесенОстатокВРубляхСПокрытиемРасч", 	Новый ОписаниеТипов("Число"));
	ТабОстатокПерерасход.Колонки.Добавить("ВыданПерерасходВРублях", 				Новый ОписаниеТипов("Число"));
	ТабОстатокПерерасход.Колонки.Добавить("ВыданПерерасходВВалюте", 				Новый ОписаниеТипов("Число"));
	ТабОстатокПерерасход.Колонки.Добавить("ВыданПерерасходВРубляхСПокрытиемБух", 	Новый ОписаниеТипов("Число"));
	ТабОстатокПерерасход.Колонки.Добавить("ВыданПерерасходВРубляхСПокрытиемРасч", 	Новый ОписаниеТипов("Число"));
	ТабОстатокПерерасход.Колонки.Добавить("НомерОрдера", 							Новый ОписаниеТипов("Строка"));
	ТабОстатокПерерасход.Колонки.Добавить("ДатаОрдера", 							Новый ОписаниеТипов("Дата"));
	ТабОстатокПерерасход.Колонки.Добавить("НомерПервичногоДокумента", 				Новый ОписаниеТипов("Строка"));
	ТабОстатокПерерасход.Колонки.Добавить("ДатаПервичногоДокумента", 				Новый ОписаниеТипов("Дата"));
	// Колонка необходима для приведения номера регистратора к выбранному виду настройки номера при печати
	ТабОстатокПерерасход.Колонки.Добавить("Регистратор"); 
	
	ТабОстатокПерерасход.Очистить();
	...
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
					...
					|	АвансовыйОтчетОстатокПерерасход.ДокументПерерасхода КАК ДокументПерерасхода,
					|	АвансовыйОтчетОстатокПерерасход.ДокументОстатка КАК ДокументОстатка,
					|	АвансовыйОтчетОстатокПерерасход.НомерСтроки КАК НомерСтроки,
					//|	АвансовыйОтчетОстатокПерерасход.Ссылка.Кратность КАК Кратность,
					//|	АвансовыйОтчетОстатокПерерасход.Ссылка.Курс КАК Курс,
					|	АвансовыйОтчетОстатокПерерасход.Ссылка КАК Ссылка
					|ПОМЕСТИТЬ ОСТАТОКПЕРЕРАСХОД
					|ИЗ
					|	Документ.ОтчетОРасходахПодотчетногоЛица.ОстатокПерерасход КАК АвансовыйОтчетОстатокПерерасход
					|ГДЕ
					|	АвансовыйОтчетОстатокПерерасход.Ссылка = &Ссылка
					|
					|ИНДЕКСИРОВАТЬ ПО
					|	Ссылка
					|;
					...
					|ВЫБРАТЬ РАЗРЕШЕННЫЕ
					|	ЖурналПроводокЕПСБУДвиженияССубконто.КФО КАК КФО,
					|	ЖурналПроводокЕПСБУДвиженияССубконто.СчетДт КАК СчетДт,
					|	ЖурналПроводокЕПСБУДвиженияССубконто.КПСДт КАК КПСДт,
					|	ЖурналПроводокЕПСБУДвиженияССубконто.КЭКДт КАК КЭКДт,
					|	ЖурналПроводокЕПСБУДвиженияССубконто.СчетКт КАК СчетКт,
					|	ЖурналПроводокЕПСБУДвиженияССубконто.КПСКт КАК КПСКт,
					|	ЖурналПроводокЕПСБУДвиженияССубконто.КЭККт КАК КЭККт,
					|	ЖурналПроводокЕПСБУДвиженияССубконто.Период КАК Период,
					|	ЖурналПроводокЕПСБУДвиженияССубконто.Регистратор КАК Регистратор,
					|	ТИПЗНАЧЕНИЯ(ЖурналПроводокЕПСБУДвиженияССубконто.Регистратор) КАК ТипРегистратор,
					|	ВЫБОР
					...
					|				ИЛИ ЖурналПроводокЕПСБУДвиженияССубконто.ВалютаКт = &ВалютаРеглУчета
					|			ТОГДА """"
					...
					|				ИЛИ ЖурналПроводокЕПСБУДвиженияССубконто.ВалютаКт = &ВалютаРеглУчета
					|			ТОГДА 0
					|		ИНАЧЕ ЖурналПроводокЕПСБУДвиженияССубконто.Сумма
					|	КОНЕЦ КАК СуммаПокрытияБух,
					|	0 КАК СуммаПокрытияРасчетная,
					|	ОСТАТОКПЕРЕРАСХОД.Ссылка КАК Ссылка
					|ПОМЕСТИТЬ ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ
					|ИЗ
					|	РегистрБухгалтерии.ЖурналПроводокЕПСБУ.ДвиженияССубконто(
					|			,
					|			,
					...
					|				И Регистратор В
					|					(ВЫБРАТЬ
					|						ОСТАТОКПЕРЕРАСХОД.ДокументПерерасхода
					|					ИЗ
					|						ОСТАТОКПЕРЕРАСХОД)
					|				И СубконтоДт1 = &Контрагент,
					|			,
					|			) КАК ЖурналПроводокЕПСБУДвиженияССубконто
					|		ЛЕВОЕ СОЕДИНЕНИЕ ОСТАТОКПЕРЕРАСХОД КАК ОСТАТОКПЕРЕРАСХОД
					|		ПО ЖурналПроводокЕПСБУДвиженияССубконто.Регистратор = ОСТАТОКПЕРЕРАСХОД.ДокументПерерасхода
					|;
					|
					|////////////////////////////////////////////////////////////////////////////////
					|ВЫБРАТЬ РАЗРЕШЕННЫЕ
					...
					|		КОГДА ЖурналПроводокЕПСБУДвиженияССубконто.ВалютаДт ЕСТЬ NULL
					|				ИЛИ ЖурналПроводокЕПСБУДвиженияССубконто.ВалютаДт = ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
					|				ИЛИ ЖурналПроводокЕПСБУДвиженияССубконто.ВалютаДт = &ВалютаРеглУчета
					|			ТОГДА """"
					|		ИНАЧЕ ЖурналПроводокЕПСБУДвиженияССубконто.ВалютаДт
					|	КОНЕЦ КАК Валюта,
					|	ВЫБОР
					...
					|	0 КАК СуммаПокрытияРасчетная,
					|	ОСТАТОКПЕРЕРАСХОД.Ссылка КАК Ссылка
					|ПОМЕСТИТЬ ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ
					|ИЗ
					|	РегистрБухгалтерии.ЖурналПроводокЕПСБУ.ДвиженияССубконто(
					|			,
					|			,
					...
					|				И Регистратор В
					|					(ВЫБРАТЬ
					|						ОСТАТОКПЕРЕРАСХОД.ДокументОстатка
					|					ИЗ
					|						ОСТАТОКПЕРЕРАСХОД)
					...
					|			) КАК ЖурналПроводокЕПСБУДвиженияССубконто
					|		ЛЕВОЕ СОЕДИНЕНИЕ ОСТАТОКПЕРЕРАСХОД КАК ОСТАТОКПЕРЕРАСХОД
					|		ПО ЖурналПроводокЕПСБУДвиженияССубконто.Регистратор = ОСТАТОКПЕРЕРАСХОД.ДокументОстатка";
	...
	Запрос.УстановитьПараметр("Счет208"			, БухгалтерскийУчет.СписокСчетов("208.00"));
	Запрос.УстановитьПараметр("СчетКор"			, БухгалтерскийУчет.СписокСчетов("201.00,210.00,304.00"));
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.ТипРегистратор
		|ИЗ
		|	ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ КАК ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ";
		...
		Если Выборка.Количество() > 0 Тогда 
			...
			Пока Выборка.Следующий() Цикл
				...
				ИмяМетаданных = Метаданные.НайтиПоТипу(Выборка.ТипРегистратор).ПолноеИмя();
				
				ДопТекстНомер = ДопТекстНомер + "
				|	КОГДА ТИПЗНАЧЕНИЯ(ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
				|		ТОГДА ВЫРАЗИТЬ(ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.Регистратор КАК " + ИмяМетаданных + ").Номер";
				
				ДопТекстДата = ДопТекстДата + "
				|	КОГДА ТИПЗНАЧЕНИЯ(ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
				|		ТОГДА ВЫРАЗИТЬ(ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.Регистратор КАК " + ИмяМетаданных + ").Дата";
			...
			ДопТекстПервичныйНомер = "
			|ВЫБОР";
			ДопТекстПервичнаяДата = "
			|ВЫБОР";
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Выборка.ТипРегистратор = Тип("ДокументСсылка.КассовоеВыбытие") Тогда
					
					ДопТекстПервичныйНомер = ДопТекстПервичныйНомер + "
					|	КОГДА ТИПЗНАЧЕНИЯ(ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
					|		ТОГДА ВЫРАЗИТЬ(ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.Регистратор КАК " + ИмяМетаданных + ").НомерПервичногоДокумента";
					
					ДопТекстПервичнаяДата = ДопТекстПервичнаяДата + "
					|	КОГДА ТИПЗНАЧЕНИЯ(ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
					|		ТОГДА ВЫРАЗИТЬ(ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.Регистратор КАК " + ИмяМетаданных + ").ДатаПервичногоДокумента";
					
				ИначеЕсли Выборка.ТипРегистратор = Тип("ДокументСсылка.ЗаявкаНаКассовыйРасход")
					ИЛИ Выборка.ТипРегистратор = Тип("ДокументСсылка.ЗаявкаНаКассовыйРасходСокращенная")
					ИЛИ Выборка.ТипРегистратор = Тип("ДокументСсылка.ПлатежноеПоручение")
					ИЛИ Выборка.ТипРегистратор = Тип("ДокументСсылка.РаспоряжениеОПеречисленииНаКартыМир") Тогда
					
					ДопТекстПервичныйНомер = ДопТекстПервичныйНомер + "
					|	КОГДА ТИПЗНАЧЕНИЯ(ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
					|		ТОГДА СостояниеПлатежноРасчетныхДокументов.НомерПодтверждающегоДокумента";
					
					ДопТекстПервичнаяДата = ДопТекстПервичнаяДата + "
					|	КОГДА ТИПЗНАЧЕНИЯ(ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
					|		ТОГДА СостояниеПлатежноРасчетныхДокументов.ДатаПодтверждающегоДокумента";
		...
		Иначе
			ДопТекстНомер 	= "NULL";
			ДопТекстДата = "NULL";
			ДопТекстПервичныйНомер 	= "NULL";
			ДопТекстПервичнаяДата = "NULL";
		КонецЕсли;
		
		// Выдача перерасхода
		Запрос.Текст = "ВЫБРАТЬ
		|	ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.КФО,
		|	ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.СчетДт,
		|	ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.КПСДт,
		|	ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.КЭКДт,
		|	ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.Регистратор КАК Регистратор,
		|   " + ДопТекстНомер + " КАК НомерОрдера,
		|   " + ДопТекстДата +  " КАК ДатаОрдера,
		|   " + ДопТекстПервичныйНомер + " КАК НомерПервичногоДокумента,
		|   " + ДопТекстПервичнаяДата +  " КАК ДатаПервичногоДокумента,
		|	СУММА(ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.СуммаРуб) КАК СуммаРуб,
		|	СУММА(ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.СуммаРуб + ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.СуммаПокрытияБух) КАК СуммаПокрытияБух,
		|	СУММА(ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.СуммаРуб + ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.СуммаПокрытияРасчетная) КАК СуммаПокрытияРасчетная,
		|	СУММА(ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.СуммаВал) КАК СуммаВал
		|ИЗ
		|	ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ КАК ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостояниеПлатежноРасчетныхДокументов КАК СостояниеПлатежноРасчетныхДокументов
		|			ПО ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.Регистратор = СостояниеПлатежноРасчетныхДокументов.ПлатежноРасчетныйДокумент
		|ГДЕ
		|	ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.Ссылка = &Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.КФО,
		|	ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.СчетДт,
		|	ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.КПСДт,
		|	ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.КЭКДт,
		|	ВЫДАНОПЕРЕРАСХОДАДВИЖЕНИЯ.Регистратор,
		|   " + ДопТекстНомер + ", 
		|   " + ДопТекстДата + ", 
		|   " + ДопТекстПервичныйНомер + ", 
		|   " + ДопТекстПервичнаяДата;
		...
		Пока  Выборка.Следующий() Цикл
			...
			Стр = ТабОстатокПерерасход.Добавить();
			Стр.ОстатокПерерасходНомерСчета				= УправлениеПечатьюБГУ.ПредставлениеРабочегоСчета(Выборка.КПСДт, Выборка.КФО, Выборка.СчетДт, Выборка.КЭКДт,, Истина);
			Стр.НомерОрдера 							= Выборка.НомерОрдера;
			Стр.ДатаОрдера 								= Выборка.ДатаОрдера;
			Стр.НомерПервичногоДокумента 				= Выборка.НомерПервичногоДокумента;
			Стр.ДатаПервичногоДокумента 				= Выборка.ДатаПервичногоДокумента;
			Стр.ВыданПерерасходВРублях 					= Выборка.СуммаРуб;
			Стр.ВыданПерерасходВВалюте 					= Выборка.СуммаВал;
			Стр.ВыданПерерасходВРубляхСПокрытиемБух 	= Выборка.СуммаПокрытияБух;
			Стр.ВыданПерерасходВРубляхСПокрытиемРасч 	= Выборка.СуммаПокрытияРасчетная;
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.ТипРегистратор
		|ИЗ
		|	ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ КАК ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ";
		...
		Если Выборка.Количество() > 0 Тогда 
			...
			Пока Выборка.Следующий() Цикл
				...
				ДопТекстНомер = ДопТекстНомер + "
				|	КОГДА ТИПЗНАЧЕНИЯ(ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
				|		ТОГДА ВЫРАЗИТЬ(ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.Регистратор КАК " + ИмяМетаданных + ").Номер";
				
				ДопТекстДата = ДопТекстДата + "
				|	КОГДА ТИПЗНАЧЕНИЯ(ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
				|		ТОГДА ВЫРАЗИТЬ(ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.Регистратор КАК " + ИмяМетаданных + ").Дата";
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Выборка.ТипРегистратор = Тип("ДокументСсылка.КассовоеПоступление") Тогда
					
					ДопТекстПервичныйНомер = ДопТекстПервичныйНомер + "
					|	КОГДА ТИПЗНАЧЕНИЯ(ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
					|		ТОГДА ВЫРАЗИТЬ(ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.Регистратор КАК " + ИмяМетаданных + ").НомерПервичногоДокумента";
					
					ДопТекстПервичнаяДата = ДопТекстПервичнаяДата + "
					|	КОГДА ТИПЗНАЧЕНИЯ(ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
					|		ТОГДА ВЫРАЗИТЬ(ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.Регистратор КАК " + ИмяМетаданных + ").ДатаПервичногоДокумента";
		...
		Запрос.Текст = "ВЫБРАТЬ
		|	ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.КФО,
		|	ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.СчетКт,
		|	ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.КПСКт,
		|	ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.КЭККт,
		|	ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.Регистратор КАК Регистратор,
		|   " + ДопТекстНомер + " КАК НомерОрдера,
		|   " + ДопТекстДата +  " КАК ДатаОрдера,
		|   " + ДопТекстПервичныйНомер + " КАК НомерПервичногоДокумента,
		|   " + ДопТекстПервичнаяДата +  " КАК ДатаПервичногоДокумента,
		|	СУММА(ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.СуммаРуб) КАК СуммаРуб,
		|	СУММА(ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.СуммаВал) КАК СуммаВал,
		|	СУММА(ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.СуммаРуб + ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.СуммаПокрытияБух) КАК СуммаПокрытияБух,
		|	СУММА(ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.СуммаРуб + ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.СуммаПокрытияРасчетная) КАК СуммаПокрытияРасчетная
		|ИЗ
		|	ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ КАК ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ
		|ГДЕ
		|	ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.Ссылка = &Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.КФО,
		|	ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.СчетКт,
		|	ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.КПСКт,
		|	ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.КЭККт,
		|	ПОЛУЧЕНОСТАТКАДВИЖЕНИЯ.Регистратор,
		|   " + ДопТекстНомер + ", 
		|   " + ДопТекстДата + ", 
		|   " + ДопТекстПервичныйНомер + ", 
		|   " + ДопТекстПервичнаяДата;
		...
		Пока Выборка.Следующий() Цикл
			...
			Стр = ТабОстатокПерерасход.Добавить();
			Стр.ОстатокПерерасходНомерСчета				= УправлениеПечатьюБГУ.ПредставлениеРабочегоСчета(
					Выборка.КПСКт, Выборка.КФО, Выборка.СчетКт, Выборка.КЭККт,, Истина);
			Стр.НомерОрдера 							= Выборка.НомерОрдера;
			Стр.ДатаОрдера 								= Выборка.ДатаОрдера;
			Стр.НомерПервичногоДокумента 				= Выборка.НомерПервичногоДокумента;
			Стр.ДатаПервичногоДокумента 				= Выборка.ДатаПервичногоДокумента;
			Стр.ВнесенОстатокВРублях 					= Выборка.СуммаРуб;
			Стр.ВнесенОстатокВВалюте 					= Выборка.СуммаВал;
			Стр.ВнесенОстатокВРубляхСПокрытиемБух	 	= Выборка.СуммаПокрытияБух;
			Стр.ВнесенОстатокВРубляхСПокрытиемРасч	 	= Выборка.СуммаПокрытияРасчетная;
			Стр.Регистратор 							= Выборка.Регистратор;
		КонецЦикла; 
				
		Возврат ТабОстатокПерерасход;	
	
	
	КонецФункции

Процедура ВывестиДопДанные(Ссылка, Контрагент, Макет, ТабличныйДокумент, ПараметрыЗаполнения)      
		
	Реквизиты = Новый Структура;
	Реквизиты.Вставить("Ссылка",          Ссылка);
	Реквизиты.Вставить("Организация",     Ссылка.Организация);
	Реквизиты.Вставить("Дата",            Ссылка.Дата);
	
	Реквизиты.Вставить("СчетРасчетов",    БухгалтерскиеОперации.СчетПоКоду("208.00", Ссылка.Дата));
		
	Реквизиты.Вставить("КурсыВалют", Ссылка.КурсыВалют.Выгрузить());
	Реквизиты.Вставить("Подразделение", Ссылка.Подразделение);
	
	Отказ = Ложь;
	
	ТаблицаРасходовПоДокументу = РасчетыСПодотчетнымиЛицами.ПодготовитьТаблицуРасходовПоОтчетуОРасходах(
		Реквизиты.Ссылка, Реквизиты, Отказ);
	
	Если НЕ Отказ Тогда
		
		ТаблицаРасходов = РасчетыСПодотчетнымиЛицами.ТаблицаРасходовПоОтчетуОРасходахСКурсамиВыдачи(
			ТаблицаРасходовПоДокументу, Реквизиты.Ссылка, Реквизиты, Отказ);
	...
	ТабАванс = Новый ТаблицаЗначений;
	ТабАванс.Колонки.Добавить("НаименованиеПоказателя", 					Новый ОписаниеТипов("Строка"));
	ТабАванс.Колонки.Добавить("НаименованиеПоказателяПервичныйДокумент", 	Новый ОписаниеТипов("Строка"));
	ТабАванс.Колонки.Добавить("СуммаАвансаВРубляхСПокрытиемРасчетная", 		Новый ОписаниеТипов("Число"));
	ТабАванс.Колонки.Добавить("СуммаАвансаВРубляхСПокрытиемБухгалтерская", 	Новый ОписаниеТипов("Число"));
	ТабАванс.Колонки.Добавить("СуммаАвансаВРублях", 						Новый ОписаниеТипов("Число"));
	ТабАванс.Колонки.Добавить("СуммаАвансаВВалюте", 						Новый ОписаниеТипов("Число"));
	ТабАванс.Колонки.Добавить("ВыданАвансДебет", 							Новый ОписаниеТипов("Строка"));
	ТабАванс.Колонки.Добавить("ВыданАвансКредит", 							Новый ОписаниеТипов("Строка"));
	ТабАванс.Колонки.Добавить("СуммаБухгалтерскаяРасчетная", 				Новый ОписаниеТипов("Число"));
	ТабАванс.Колонки.Добавить("СуммаБухгалтерская", 						Новый ОписаниеТипов("Число"));
	ТабАванс.Колонки.Добавить("ИзрасходованоДебет",							Новый ОписаниеТипов("Строка"));
	ТабАванс.Колонки.Добавить("ИзрасходованоКредит",						Новый ОписаниеТипов("Строка"));
	ТабАванс.Колонки.Добавить("СуммаВРубляхПоКурсуАванса",					Новый ОписаниеТипов("Число"));
	...
	|	Таблица.КПС КАК КПС,
	|	Таблица.КПСДт КАК КПСДт,
	|	Таблица.КФО КАК КФО,
	|	Таблица.КЭК КАК КЭК,
	|	Таблица.КЭКДт КАК КЭКДт,
	|	Таблица.Количество КАК Количество,
	|	Таблица.Контрагент КАК Контрагент,
	|	Таблица.НаименованиеРасхода КАК НаименованиеРасхода,
	|	Таблица.НомерСтроки КАК НомерСтроки,
	|	Таблица.ОстатокРасчетов КАК ОстатокРасчетов,
	|	Таблица.ОтражениеВУСН КАК ОтражениеВУСН,
	|	Таблица.Подразделение КАК Подразделение,
	|	Таблица.ПодразделениеДт КАК ПодразделениеДт,
	|	Таблица.ПредъявленСФ КАК ПредъявленСФ,
	|	Таблица.РасчетыВвалюте КАК РасчетыВвалюте,
	|	Таблица.СинонимСписка КАК СинонимСписка,
	|	Таблица.СубконтоДебета1 КАК СубконтоДебета1,
	|	Таблица.СубконтоДебета2 КАК СубконтоДебета2,
	|	Таблица.СубконтоДебета3 КАК СубконтоДебета3,
	|	Таблица.СубконтоНУ1 КАК СубконтоНУ1,
	|	Таблица.СубконтоНУ2 КАК СубконтоНУ2,
	|	Таблица.СубконтоНУ3 КАК СубконтоНУ3,
	|	Таблица.СуммаНДС КАК СуммаНДС,
	|	Таблица.СуммаРАсчетов КАК СуммаРАсчетов,
	|	Таблица.СуммаРуб КАК СуммаРуб,
	|	Таблица.СуммаРубПоКурсуАванса КАК СуммаРубПоКурсуАванса,
	|	Таблица.СчетДебета КАК СчетДебета,
	|	Таблица.СчетНУ КАК СчетНУ,
	|	Таблица.СчетРасчетов КАК СчетРасчетов
	|ПОМЕСТИТЬ ТАБЛИЦА
	|ИЗ
	|	&ТаблицаРасходов КАК Таблица
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|	ТАБЛИЦА.НомерСтроки КАК НомерСтроки,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДокументДата,
	|	"""" КАК ДокументНомер,
	|	"""" КАК КомуЗаЧто,
	|	ТАБЛИЦА.КФО КАК КФО,
	|	ТАБЛИЦА.КПС КАК КПС,
	|	ВЫБОР
	|		КОГДА ТАБЛИЦА.КПСДт = ЗНАЧЕНИЕ(Справочник.КлассификационныеПризнакиСчетов.ПустаяСсылка)
	|			ТОГДА ТАБЛИЦА.КПС
	|		ИНАЧЕ ТАБЛИЦА.КПСДт
	|	КОНЕЦ КАК КПСДт,
	|	ТАБЛИЦА.КЭК КАК КЭК,
	|	ЛОЖЬ КАК СуммаВключаетНДС,
	|	ТАБЛИЦА.СуммаРуб КАК Сумма,
	|	ТАБЛИЦА.СуммаНДС КАК СуммаНДС,
	|	0 КАК СуммаВВалюте,
	|	ТАБЛИЦА.СуммаРуб КАК СуммаПоОтчету,
	|	0 КАК СуммаВВалютеПоОтчету,
	|	ТАБЛИЦА.СчетДебета КАК СчетДебета,
	|	ТАБЛИЦА.СчетРасчетов КАК СчетКредита,
	|	ТАБЛИЦА.СчетНУ КАК СчетУчетаНДС,
	|	ТАБЛИЦА.СубконтоДебета1 КАК СубконтоДебета1,
	|	ТАБЛИЦА.СубконтоДебета2 КАК СубконтоДебета2,
	|	ТАБЛИЦА.СубконтоДебета3 КАК СубконтоДебета3,
	|	ТАБЛИЦА.СуммаРуб КАК СуммаПокрытия,
	|	ТАБЛИЦА.СуммаРуб КАК СуммаПокрытияПоОтчету,
	|	ТАБЛИЦА.СуммаРАсчетов КАК СуммаРасчетов,
	|	ТАБЛИЦА.СуммаРАсчетов КАК СуммаРасчетовПоОтчету,
	|	0 КАК РублевыйЭквивалент,
	|	0 КАК РублевыйЭквивалентПоОтчету,
	|	1 КАК Кратность,
	|	1 КАК Курс,
	|	ТАБЛИЦА.Валюта КАК ВалютаДокумента1,
	|	ВЫБОР
	|		КОГДА ТАБЛИЦА.СуммаРуб = 0
	|			ТОГДА ТАБЛИЦА.Валюта
	|		ИНАЧЕ &ВалютаРеглУчета
	|	КОНЕЦ КАК ВалютаДокумента,
	|	ВЫБОР
	|		КОГДА ТАБЛИЦА.Валюта ЕСТЬ NULL
	|				ИЛИ ТАБЛИЦА.Валюта = &ВалютаРеглУчета
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	...
	|	Документ.ОтчетОРасходахПодотчетногоЛица.Получено КАК ОтчетОРасходахПодотчетногоЛицаПолучено
	|ГДЕ
	|	ОтчетОРасходахПодотчетногоЛицаПолучено.Ссылка = &Ссылка
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	...
	|	ЖурналПроводокЕПСБУДвиженияССубконто.Регистратор КАК Регистратор,
	|	ТИПЗНАЧЕНИЯ(ЖурналПроводокЕПСБУДвиженияССубконто.Регистратор) КАК ТипРегистратор,
	|	ЖурналПроводокЕПСБУДвиженияССубконто.ВалютаКт КАК Валюта,
	|	ВЫБОР
	|		КОГДА ЖурналПроводокЕПСБУДвиженияССубконто.ВалютаКт ЕСТЬ NULL
	|				ИЛИ ЖурналПроводокЕПСБУДвиженияССубконто.ВалютаКт = ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
	|				ИЛИ ЖурналПроводокЕПСБУДвиженияССубконто.ВалютаКт = &ВалютаРеглУчета
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ПОЛУЧЕНОАВАНСОВ.НомерСтроки,
	|	ЖурналПроводокЕПСБУДвиженияССубконто.НомерСтроки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОперацииДокументов.Документ КАК Документ,
	|	ОперацииДокументов.ИсправлениеОшибки КАК ИсправлениеОшибки,
	|	ОперацииДокументов.ВыявительОшибки КАК ВыявительОшибки,
	|	ОперацииДокументов.ПричинаОшибки КАК ПричинаОшибки
	|ИЗ
	|	РегистрСведений.ОперацииДокументов КАК ОперацииДокументов
	|ГДЕ
	|	ОперацииДокументов.Документ = &Ссылка
	|	И ОперацииДокументов.ИсправлениеОшибки <> ЗНАЧЕНИЕ(Перечисление.ПорядокИсправленияОшибокПрошлыхЛет.ПустаяСсылка)
	|	И ОперацииДокументов.ИсправлениеОшибки <> ЗНАЧЕНИЕ(Перечисление.ПорядокИсправленияОшибокПрошлыхЛет.НеОшибка)";  
	
	ПакетЗапросов = Запрос.ВыполнитьПакет();
	ТаблицаИзрасходовано = ПакетЗапросов[1].Выгрузить();
	ОстаткиАвансов = ПакетЗапросов[3].Выгрузить();
	ОперацииСИсправлениемОшибок = ПакетЗапросов[4].Выгрузить();
	
	ОписаниеТипаСуммы = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(18, 2, ДопустимыйЗнак.Любой));
	
	ТаблицаИзрасходовано.Колонки.Добавить("СуммаРубПоКурсуАванса", 			ОписаниеТипаСуммы);
	ТаблицаИзрасходовано.Колонки.Добавить("ОстатокРасчетов", 				ОписаниеТипаСуммы);
	ТаблицаИзрасходовано.Колонки.Добавить("СуммаРубПоКурсуАвансаПоОтчету", 	ОписаниеТипаСуммы);
	ТаблицаИзрасходовано.Колонки.Добавить("ОстатокРасчетовПоОтчету", 		ОписаниеТипаСуммы);
	...
	Для Каждого СтрокаДокумента Из ТаблицаИзрасходовано Цикл
		...
		Если ОстаткиАвансов.Количество() > 0 Тогда
			...
			Отбор = Новый Структура;
			Отбор.Вставить("Ссылка",СтрокаДокумента.Ссылка);
			Отбор.Вставить("КФО",СтрокаДокумента.КФО);
			Отбор.Вставить("СчетДт",СтрокаДокумента.СчетКредита);
			Отбор.Вставить("КПСДт",СтрокаДокумента.КПС);
			
			СвойстваСчетаРасчетов = БухгалтерскийУчетПовтИсп.ПолучитьСвойстваСчета(СтрокаДокумента.СчетКредита);
			Если ПолучитьФункциональнуюОпцию("ИспользоватьВалютныйУчет") И СвойстваСчетаРасчетов.Валютный Тогда
				Отбор.Вставить("Валюта", СтрокаДокумента.ВалютаДокумента);
			КонецЕсли;
	...
	КолонкиГруппировки = 
		"Ссылка, НомерСтроки, ДокументДата, ДокументНомер, КомуЗаЧто, КФО, КПС, КПСДт,
		|КЭК, Сумма, СуммаНДС, СуммаВВалюте, СуммаПоОтчету, СуммаВВалютеПоОтчету,
		|СчетДебета, СчетКредита, СчетУчетаНДС, СуммаВключаетНДС,
		|СубконтоДебета1, СубконтоДебета2, СубконтоДебета3,
		|СуммаПокрытия, СуммаПокрытияПоОтчету, 
		|Кратность,Курс,ВалютаДокумента,РасчетыВВалюте";
	КолонкиСуммирования = 	"РублевыйЭквивалент, СуммаРасчетов, СуммаРубПоКурсуАванса,
							|РублевыйЭквивалентПоОтчету, СуммаРасчетовПоОтчету, СуммаРубПоКурсуАвансаПоОтчету";
	ТаблицаРасчетов.Свернуть(КолонкиГруппировки,КолонкиСуммирования);
	
	Запрос.УстановитьПараметр("АвансовыйОтчетИзрасходовано",ТаблицаРасчетов);
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
					...
					|	АвансовыйОтчет.ИФО КАК ИФО,
					|	АвансовыйОтчет.Организация КАК Организация,
					|	ВЫБОР
					|		КОГДА АвансовыйОтчет.Организация.НаименованиеПолное = """"
					|			ТОГДА ВЫБОР
					|					КОГДА АвансовыйОтчет.Организация.НаименованиеСокращенное = """"
					|						ТОГДА АвансовыйОтчет.Организация.Наименование
					|					ИНАЧЕ АвансовыйОтчет.Организация.НаименованиеСокращенное
					|				КОНЕЦ
					|		ИНАЧЕ АвансовыйОтчет.Организация.НаименованиеПолное
					|	КОНЕЦ КАК НаименованиеОрганизации,
					|	АвансовыйОтчет.Организация.КодОКПО КАК КодОКПООрганизации,
					|	АвансовыйОтчет.Организация.ИНН КАК ИНН,
					|	АвансовыйОтчет.Организация.КПП КАК КПП,
					//|	АвансовыйОтчет.Подразделение КАК Подразделение,
					//|	АвансовыйОтчет.Подразделение.Наименование КАК СтруктурноеПодразделение,
					//|	АвансовыйОтчет.Подразделение.Код КАК КодСтруктурногоПодразделения,
					//|	АвансовыйОтчет.Сотрудник КАК Сотрудник,
					//|	АвансовыйОтчет.Сотрудник.Код КАК ТабельныйНомер,
					//|	АвансовыйОтчет.Сотрудник.Контрагент КАК СотрудникКонтрагент,
					//|	АвансовыйОтчет.ВалютаДокумента КАК ВалютаДокумента,
					//|	ЕСТЬNULL(АвансовыйОтчет.Курс, 0) КАК Курс,
					//|	ЕСТЬNULL(АвансовыйОтчет.Кратность, 0) КАК Кратность,
					//|	ВЫБОР
					//|		КОГДА АвансовыйОтчет.ВалютаДокумента = &ВалютаРеглУчета
					//|				ИЛИ АвансовыйОтчет.ВалютаДокумента ЕСТЬ NULL
					//|			ТОГДА """"
					...
					//|	КОНЕЦ КАК КодВалютыПоОКВ,
					//|	АвансовыйОтчет.НазначениеАванса КАК НазначениеАванса,
					|	АвансовыйОтчет.ОстатокПредыдущегоАванса КАК ОстатокПредыдущегоАванса,
					|	АвансовыйОтчет.ОстатокПредыдущегоАвансаВВалюте КАК ОстатокПредыдущегоАвансаВВалюте,
					|	АвансовыйОтчет.ПерерасходПредыдущегоАванса КАК ПерерасходПредыдущегоАванса,
					|	АвансовыйОтчет.ПерерасходПредыдущегоАвансаВвалюте КАК ПерерасходПредыдущегоАвансаВвалюте
					//|	АвансовыйОтчет.Руководитель КАК Руководитель,
					//|	АвансовыйОтчет.ГлБухгалтер КАК ГлБухгалтер,
					//|	АвансовыйОтчет.РуководительПодразделения КАК РуководительПодразделения,
					//|	АвансовыйОтчет.Бухгалтер КАК Бухгалтер,
					//|	АвансовыйОтчет.БухгалтерКассир КАК БухгалтерКассир,
					...
					|ИЗ
					|	Документ.ОтчетОРасходахПодотчетногоЛица КАК АвансовыйОтчет
					|ГДЕ
					...
					|////////////////////////////////////////////////////
				   |ВЫБРАТЬ
	               |	АвансовыйОтчетИзрасходовано.Ссылка КАК Ссылка,
	               |	АвансовыйОтчетИзрасходовано.НомерСтроки КАК НомерСтроки,
	               |	АвансовыйОтчетИзрасходовано.ДокументДата КАК ДокументДата,
	               |	АвансовыйОтчетИзрасходовано.ДокументНомер КАК ДокументНомер,
	               |	АвансовыйОтчетИзрасходовано.КомуЗаЧто КАК КомуЗаЧто,
	               |	АвансовыйОтчетИзрасходовано.КФО КАК КФО,
	               |	АвансовыйОтчетИзрасходовано.КПС КАК КПС,
	               |	АвансовыйОтчетИзрасходовано.КПСДт КАК КПСДт,
	               |	АвансовыйОтчетИзрасходовано.КЭК КАК КЭК,
	               |	АвансовыйОтчетИзрасходовано.Сумма КАК Сумма,
	               |	АвансовыйОтчетИзрасходовано.СуммаНДС КАК СуммаНДС,
	               |	АвансовыйОтчетИзрасходовано.СуммаВВалюте КАК СуммаВВалюте,
	               |	АвансовыйОтчетИзрасходовано.СуммаПоОтчету КАК СуммаПоОтчету,
	               |	АвансовыйОтчетИзрасходовано.СуммаВВалютеПоОтчету КАК СуммаВВалютеПоОтчету,
	               |	АвансовыйОтчетИзрасходовано.СчетДебета КАК СчетДебета,
	               |	АвансовыйОтчетИзрасходовано.СчетКредита КАК СчетКредита,
	               |	АвансовыйОтчетИзрасходовано.СчетУчетаНДС КАК СчетУчетаНДС,
	               |	АвансовыйОтчетИзрасходовано.СубконтоДебета1 КАК СубконтоДебета1,
	               |	АвансовыйОтчетИзрасходовано.СубконтоДебета2 КАК СубконтоДебета2,
	               |	АвансовыйОтчетИзрасходовано.СубконтоДебета3 КАК СубконтоДебета3,
	               |	АвансовыйОтчетИзрасходовано.СуммаПокрытия КАК СуммаПокрытия,
	               |	АвансовыйОтчетИзрасходовано.СуммаПокрытияПоОтчету КАК СуммаПокрытияПоОтчету,
	               |	АвансовыйОтчетИзрасходовано.СуммаРасчетов КАК СуммаРасчетов,
	               |	АвансовыйОтчетИзрасходовано.РублевыйЭквивалент КАК РублевыйЭквивалент,
	               |	АвансовыйОтчетИзрасходовано.СуммаРубПоКурсуАванса КАК СуммаРубПоКурсуАванса,
	               |	АвансовыйОтчетИзрасходовано.СуммаРасчетовПоОтчету КАК СуммаРасчетовПоОтчету,
	               |	АвансовыйОтчетИзрасходовано.РублевыйЭквивалентПоОтчету КАК РублевыйЭквивалентПоОтчету,
	               |	АвансовыйОтчетИзрасходовано.СуммаРубПоКурсуАвансаПоОтчету КАК СуммаРубПоКурсуАвансаПоОтчету,
	               |	АвансовыйОтчетИзрасходовано.Кратность КАК Кратность,
	               |	АвансовыйОтчетИзрасходовано.Курс КАК Курс,
	               |	АвансовыйОтчетИзрасходовано.ВалютаДокумента КАК ВалютаДокумента
	               |ПОМЕСТИТЬ ИЗРАСХОДОВАНО_ВНЕШНЯЯ
	               |ИЗ
	               |	&АвансовыйОтчетИзрасходовано КАК АвансовыйОтчетИзрасходовано
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	               ...
	               |	Документ.ОтчетОРасходахПодотчетногоЛица.ТоварыРаботыУслуги КАК ОтчетОРасходахПодотчетногоЛицаТоварыРаботыУслуги
	               |ГДЕ
	               |	ОтчетОРасходахПодотчетногоЛицаТоварыРаботыУслуги.Ссылка = &Ссылка
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	ПриходныйДокумент
	               |;
	               ...
	               |ИЗ
	               |	ИЗРАСХОДОВАНО_ВНЕШНЯЯ КАК АвансовыйОтчетИзрасходовано
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |				ИЛИ ЖурналПроводокЕПСБУДвиженияССубконто.ВалютаКт = &ВалютаРеглУчета
	               |			ТОГДА 0
	               |		ИНАЧЕ ЖурналПроводокЕПСБУДвиженияССубконто.ВалютнаяСуммаКт * ПОЛУЧЕНОАВАНСОВ.Курс / ВЫБОР
	               |				КОГДА ПОЛУЧЕНОАВАНСОВ.Кратность = 0
	               |					ТОГДА 1
	               |				ИНАЧЕ ПОЛУЧЕНОАВАНСОВ.Кратность
	               |			КОНЕЦ
	               |	КОНЕЦ КАК СуммаПокрытияРасчетная,
	               |	ПОЛУЧЕНОАВАНСОВ.Ссылка КАК Ссылка
	               |ПОМЕСТИТЬ ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ
	               |ИЗ
	               |	РегистрБухгалтерии.ЖурналПроводокЕПСБУ.ДвиженияССубконто(
	               |			,
	               |			,
	               ...
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ ПОЛУЧЕНОАВАНСОВ
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ ОСТАТОКПЕРЕРАСХОД
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ИЗРАСХОДОВАНО.Ссылка КАК Ссылка,
	               |	СУММА(ИЗРАСХОДОВАНО.Сумма) КАК Сумма,
	               |	СУММА(ИЗРАСХОДОВАНО.СуммаНДС) КАК СуммаНДС,
	               |	СУММА(ИЗРАСХОДОВАНО.СуммаВВалюте) КАК СуммаВВалюте,
	               |	СУММА(ИЗРАСХОДОВАНО.СуммаПокрытия) КАК СуммаПокрытия,
	               |	СУММА(ИЗРАСХОДОВАНО.СуммаРубПоКурсуАванса) КАК СуммаРубПоКурсуАванса
	               |ПОМЕСТИТЬ ИЗРАСХОДОВАНОСГРУППИРОВАННАЯ
	               |ИЗ
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Ссылка КАК Ссылка,
	               |	СУММА(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаРуб) КАК СуммаРуб,
	               |	СУММА(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаВал) КАК СуммаВал,
	               |	СУММА(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаПокрытияБух) КАК СуммаПокрытияБух,
	               |	СУММА(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаПокрытияРасчетная) КАК СуммаПокрытияРасчетная
	               |ПОМЕСТИТЬ ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯСГРУППИРОВАННАЯ
	               |ИЗ
	               |	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ КАК ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Ссылка
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ  
				   ...
				   |	АВАНСОВЫЙОТЧЕТ.Дата КАК Дата,
				   |	АВАНСОВЫЙОТЧЕТ.Номер КАК Номер,
				   |	АВАНСОВЫЙОТЧЕТ.Ссылка.ОстатокПредыдущегоАванса КАК ОстатокПредыдущегоАванса,
				   |	АВАНСОВЫЙОТЧЕТ.Ссылка.ПерерасходПредыдущегоАванса КАК ПерерасходПредыдущегоАванса,
				   |	АВАНСОВЫЙОТЧЕТ.Ссылка.ОстатокПредыдущегоАвансаВВалюте КАК ОстатокПредыдущегоАвансаВВалюте,
				   |	АВАНСОВЫЙОТЧЕТ.Ссылка.ПерерасходПредыдущегоАвансаВВалюте КАК ПерерасходПредыдущегоАвансаВВалюте,
	               |	ЕСТЬNULL(ИЗРАСХОДОВАНО.Сумма, 0) КАК ИзрасходованоРуб,
	               |	ЕСТЬNULL(ИЗРАСХОДОВАНО.СуммаНДС, 0) КАК ИзрасходованоНДС,
	               |	ЕСТЬNULL(ИЗРАСХОДОВАНО.СуммаВВалюте, 0) КАК ИзрасходованоВал,
	               |	ЕСТЬNULL(ИЗРАСХОДОВАНО.СуммаПокрытия, 0) КАК ИзрасходованоСуммаПокрытия,
	               |	ЕСТЬNULL(ИЗРАСХОДОВАНО.СуммаРубПоКурсуАванса, 0) КАК ИзрасходованоРубПоКурсуАванса,
	               |	ЕСТЬNULL(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаРуб, 0) КАК ПолученоРуб,
	               |	ЕСТЬNULL(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаВал, 0) КАК ПолученоВал,
	               |	ЕСТЬNULL(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаПокрытияБух, 0) КАК СуммаПокрытияБух,
	               |	ЕСТЬNULL(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаПокрытияРасчетная, 0) КАК СуммаПокрытияРасчетная,
	               |	ИЗРАСХОДОВАНО.Сумма КАК Сумма,
	               |	ИЗРАСХОДОВАНО.СуммаВВалюте КАК СуммаВВалюте
	               |ИЗ  
				   |	АВАНСОВЫЙОТЧЕТ КАК АВАНСОВЫЙОТЧЕТ
					|		ЛЕВОЕ СОЕДИНЕНИЕ ИЗРАСХОДОВАНОСГРУППИРОВАННАЯ КАК ИЗРАСХОДОВАНО
					|		ПО АВАНСОВЫЙОТЧЕТ.Ссылка = ИЗРАСХОДОВАНО.Ссылка
					|		ЛЕВОЕ СОЕДИНЕНИЕ ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯСГРУППИРОВАННАЯ КАК ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ
					|		ПО АВАНСОВЫЙОТЧЕТ.Ссылка = ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Ссылка";
	...
	ВыборкаАО = Результат.Выбрать();
	ВыборкаАО.Следующий();
	
	СтруктураДанных = Новый Структура;
	Для Каждого Колонка Из Результат.Колонки Цикл
		СтруктураДанных.Вставить(Колонка.Имя, ВыборкаАО[Колонка.Имя]);
	КонецЦикла; 

	
	Стр = ТабАванс.Добавить();
		Стр.НаименованиеПоказателя = "     остаток";
		Стр.НаименованиеПоказателяПервичныйДокумент = "     остаток";
		Стр.СуммаАвансаВРублях 							= Ссылка.ОстатокПредыдущегоАванса;
		Стр.СуммаАвансаВВалюте 							= Ссылка.ОстатокПредыдущегоАвансаВВалюте;
		Стр.СуммаАвансаВРубляхСПокрытиемРасчетная 		= Ссылка.ОстатокПредыдущегоАванса;
		Стр.СуммаАвансаВРубляхСПокрытиемБухгалтерская 	= Ссылка.ОстатокПредыдущегоАванса;
		
		Стр = ТабАванс.Добавить();
		Стр.НаименованиеПоказателя = "     перерасход";
		Стр.НаименованиеПоказателяПервичныйДокумент = "     перерасход";
		Стр.СуммаАвансаВРублях 							= Ссылка.ПерерасходПредыдущегоАванса;
		Стр.СуммаАвансаВВалюте 							= Ссылка.ПерерасходПредыдущегоАвансаВвалюте;
		Стр.СуммаАвансаВРубляхСПокрытиемРасчетная 		= Ссылка.ПерерасходПредыдущегоАванса;
		Стр.СуммаАвансаВРубляхСПокрытиемБухгалтерская 	= Ссылка.ПерерасходПредыдущегоАванса;
		
		Стр = ТабАванс.Добавить();
		Стр.НаименованиеПоказателя = "Получен аванс";
		Стр.НаименованиеПоказателяПервичныйДокумент = "Получен аванс";

		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.ТипРегистратор
		|ИЗ
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ КАК ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ";
		...
		Если Выборка.Количество() > 0 Тогда 
			...
			ДопТекстПервичнаяДата = "
			|ВЫБОР";
			ДопТекстПервичныйИмя = "
			|ВЫБОР";
			
			Пока Выборка.Следующий() Цикл
				
				МетаданныеРг =  Метаданные.НайтиПоТипу(Выборка.ТипРегистратор);
				
				ИмяМетаданных = МетаданныеРг.ПолноеИмя();
				ИмяДокумента =  СтрЗаменить(МетаданныеРг.Представление(), """", """""");
				
				ДопТекстНомер = ДопТекстНомер + "
				|	КОГДА ТИПЗНАЧЕНИЯ(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
				|		ТОГДА ВЫРАЗИТЬ(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Регистратор КАК " + ИмяМетаданных + ").Номер";
				
				ДопТекстДата = ДопТекстДата + "
				|	КОГДА ТИПЗНАЧЕНИЯ(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
				|		ТОГДА ВЫРАЗИТЬ(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Регистратор КАК " + ИмяМетаданных + ").Дата";
				
				ДопТекстИмя = ДопТекстИмя + "
				|	КОГДА ТИПЗНАЧЕНИЯ(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
				|		ТОГДА " + """" + ИмяДокумента + """";
				...
				Если Выборка.ТипРегистратор = Тип("ДокументСсылка.КассовоеВыбытие") Тогда
					...
					ДопТекстПервичныйНомер = ДопТекстПервичныйНомер + "
					|	КОГДА ТИПЗНАЧЕНИЯ(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
					|		ТОГДА ВЫРАЗИТЬ(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Регистратор КАК " + ИмяМетаданных + ").НомерПервичногоДокумента";
					
					ДопТекстПервичнаяДата = ДопТекстПервичнаяДата + "
					|	КОГДА ТИПЗНАЧЕНИЯ(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
					|		ТОГДА ВЫРАЗИТЬ(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Регистратор КАК " + ИмяМетаданных + ").ДатаПервичногоДокумента";
					
					ДопТекстПервичныйИмя = ДопТекстПервичныйИмя + "
					|	КОГДА ТИПЗНАЧЕНИЯ(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
					|		ТОГДА ВЫРАЗИТЬ(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Регистратор КАК " + ИмяМетаданных + ").ВидПервичногоДокумента";
				...
				ИначеЕсли Выборка.ТипРегистратор = Тип("ДокументСсылка.ЗаявкаНаКассовыйРасход")
					...
					ДопТекстПервичныйИмя = ДопТекстПервичныйИмя + "
					|	КОГДА ТИПЗНАЧЕНИЯ(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Регистратор) = ТИП(" + ИмяМетаданных + ")
					|		ТОГДА СостояниеПлатежноРасчетныхДокументов.ВидПодтверждающегоДокумента";
		...
		Иначе
			...
			ДопТекстПервичныйНомер = "NULL";
			ДопТекстПервичнаяДата = "NULL";
			ДопТекстПервичныйИмя = "NULL";
		КонецЕсли;
		
		Запрос.Текст ="ВЫБРАТЬ
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Регистратор КАК Документ,
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Период,
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Валюта,
		|   " + ДопТекстНомер + " КАК НомерОрдера,
		|   " + ДопТекстДата +  " КАК ДатаОрдера,
		|   " + ДопТекстИмя +  " КАК НаименованиеДокумента,
		|   " + ДопТекстПервичныйНомер + " КАК НомерПервичногоДокумента,
		|   " + ДопТекстПервичнаяДата +  " КАК ДатаПервичногоДокумента,
		|   " + ДопТекстПервичныйИмя +  " КАК НаименованиеПервичногоДокумента,
		|	СУММА(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаРуб) КАК СуммаРуб,
		|	СУММА(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаВал) КАК СуммаВал,
		|	СУММА(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаПокрытияБух) КАК СуммаПокрытияБух,
		|	СУММА(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаПокрытияРасчетная) КАК СуммаПокрытияРасчетная
		...
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ КАК ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостояниеПлатежноРасчетныхДокументов КАК СостояниеПлатежноРасчетныхДокументов
		|			ПО ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Регистратор = СостояниеПлатежноРасчетныхДокументов.ПлатежноРасчетныйДокумент
		|ГДЕ
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Ссылка = &Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Период,
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Валюта,
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Регистратор,
		|   " + ДопТекстНомер + ",
		|   " + ДопТекстДата + ",
		|   " + ДопТекстИмя+ ",
		|   " + ДопТекстПервичныйНомер + ",
		...
		Пока Выборка.Следующий() Цикл
			...
			НомерОрдера = ОбщегоНазначенияБГУ.ПолучитьНомерДляПечатиПоНастройке(Выборка.Документ, Выборка.НомерОрдера);
			
			Стр.НаименованиеПоказателя 						= Выборка.НаименованиеДокумента + " от " +Формат(Выборка.ДатаОрдера,"ДФ=dd.MM.yyyy") + " № " + НомерОрдера;
			Стр.НаименованиеПоказателяПервичныйДокумент 	= Выборка.НаименованиеПервичногоДокумента + " от " +Формат(Выборка.ДатаПервичногоДокумента,"ДФ=dd.MM.yyyy") + " № " + Выборка.НомерПервичногоДокумента;
			Стр.СуммаАвансаВРублях 							= Выборка.СуммаРуб;
			Стр.СуммаАвансаВВалюте 							= Выборка.СуммаВал;
			Стр.СуммаАвансаВРубляхСПокрытиемРасчетная 		= Выборка.СуммаРуб + Выборка.СуммаПокрытияРасчетная;	
			Стр.СуммаАвансаВРубляхСПокрытиемБухгалтерская 	= Выборка.СуммаРуб + Выборка.СуммаПокрытияБух;
			
		КонецЦикла;
		
		Стр = ТабАванс.Добавить();
		Стр.НаименованиеПоказателя 						= "Итого получено";
		Стр.НаименованиеПоказателяПервичныйДокумент 	= "Итого получено";
		Стр.СуммаАвансаВРублях 							= СтруктураДанных.ПолученоРуб;
		Стр.СуммаАвансаВВалюте 							= СтруктураДанных.ПолученоВал;
		Стр.СуммаАвансаВРубляхСПокрытиемРасчетная 		= СтруктураДанных.ПолученоРуб + СтруктураДанных.СуммаПокрытияРасчетная;	
		Стр.СуммаАвансаВРубляхСПокрытиемБухгалтерская 	= СтруктураДанных.ПолученоРуб + СтруктураДанных.СуммаПокрытияБух;
		
		
		Стр = ТабАванс.Добавить();
		Стр.НаименованиеПоказателя 						= "Израсходовано";
		Стр.НаименованиеПоказателяПервичныйДокумент 	= "Израсходовано";
		Стр.СуммаАвансаВРублях 							= СтруктураДанных.ИзрасходованоРуб + СтруктураДанных.ИзрасходованоНДС;
		Стр.СуммаАвансаВВалюте 							= СтруктураДанных.ИзрасходованоВал;
		Стр.СуммаАвансаВРубляхСПокрытиемРасчетная 		= СтруктураДанных.ИзрасходованоРуб + СтруктураДанных.ИзрасходованоСуммаПокрытия + СтруктураДанных.ИзрасходованоНДС;	
		Стр.СуммаАвансаВРубляхСПокрытиемБухгалтерская 	= СтруктураДанных.ИзрасходованоРубПоКурсуАванса;
		
		СуммаРезультатРуб 							= СтруктураДанных.ОстатокПредыдущегоАванса - СтруктураДанных.ПерерасходПредыдущегоАванса + СтруктураДанных.ПолученоРуб - СтруктураДанных.ИзрасходованоРуб - СтруктураДанных.ИзрасходованоНДС;
		СуммаРезультатРубСПокрытиемРасчетная 		= СтруктураДанных.ОстатокПредыдущегоАванса - СтруктураДанных.ПерерасходПредыдущегоАванса + СтруктураДанных.ПолученоРуб + СтруктураДанных.СуммаПокрытияРасчетная - СтруктураДанных.ИзрасходованоРуб - СтруктураДанных.ИзрасходованоНДС - СтруктураДанных.ИзрасходованоСуммаПокрытия;
		СуммаРезультатРубСПокрытиемБухгалтерская 	= СтруктураДанных.ОстатокПредыдущегоАванса - СтруктураДанных.ПерерасходПредыдущегоАванса + СтруктураДанных.ПолученоРуб + СтруктураДанных.СуммаПокрытияБух - СтруктураДанных.ИзрасходованоРубПоКурсуАванса;
		
		СуммаРезультатВал = СтруктураДанных.ОстатокПредыдущегоАвансаВВалюте - СтруктураДанных.ПерерасходПредыдущегоАвансаВвалюте + СтруктураДанных.ПолученоВал - СтруктураДанных.ИзрасходованоВал;
		
		Стр = ТабАванс.Добавить();
		Стр.НаименованиеПоказателя 						= "Остаток";
		Стр.НаименованиеПоказателяПервичныйДокумент 	= "Остаток";
		Стр.СуммаАвансаВРублях 							= ?(СуммаРезультатРуб > 0, СуммаРезультатРуб, 0);
		Стр.СуммаАвансаВВалюте 							= ?(СуммаРезультатВал > 0, СуммаРезультатВал, 0);
		Стр.СуммаАвансаВРубляхСПокрытиемРасчетная 		= ?(СуммаРезультатРубСПокрытиемРасчетная > 0, СуммаРезультатРубСПокрытиемРасчетная, 0);
		Стр.СуммаАвансаВРубляхСПокрытиемБухгалтерская 	= ?(СуммаРезультатРубСПокрытиемБухгалтерская > 0, СуммаРезультатРубСПокрытиемБухгалтерская, 0);
		...
		Запрос.Текст = "ВЫБРАТЬ
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.КФО,
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СчетДт,
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.КПСДт,
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.КЭКДт,
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СчетКт,
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.КПСКт,
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.КЭККт,
		|	СУММА(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаРуб) КАК СуммаРуб,
		|	СУММА(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаВал) КАК СуммаВал,
		|	СУММА(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаРуб + ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаПокрытияБух) КАК СуммаПокрытияБух,
		|	СУММА(ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаРуб + ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.СуммаПокрытияРасчетная) КАК СуммаПокрытияРасчетная
		|ИЗ
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ КАК ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ
		|ГДЕ
		|	ПОЛУЧЕНОАВАНСОВДВИЖЕНИЯ.Ссылка = &Ссылка
		...
		Для Каждого Стр Из ТабАванс Цикл
			
			Если Выборка.Следующий() Тогда
				
				Стр.ВыданАвансДебет 			= УправлениеПечатьюБГУ.ПредставлениеРабочегоСчета(Выборка.КПСДт, Выборка.КФО, Выборка.СчетДт, Выборка.КЭКДт,, Истина);
				Стр.ВыданАвансКредит			= УправлениеПечатьюБГУ.ПредставлениеРабочегоСчета(Выборка.КПСКт, Выборка.КФО, Выборка.СчетКт, Выборка.КЭККт,, Истина);
				Стр.СуммаБухгалтерская   		= Выборка.СуммаПокрытияБух;
				Стр.СуммаБухгалтерскаяРасчетная = Выборка.СуммаПокрытияРасчетная;
		...
		СтруктураДанных.Вставить("ИтогоСуммаБухгалтерская", ТабАванс.Итог("СуммаБухгалтерская"));
		СтруктураДанных.Вставить("ИтогоСуммаБухгалтерскаяРасчетная", ТабАванс.Итог("СуммаБухгалтерскаяРасчетная"));
		...
		Область = Макет.ПолучитьОбласть("ЗаголовокАванс");
		
		Область.Параметры.Заполнить(СтруктураДанных);
		
		РегистрыСведений.НастройкиПечатиПрефиксаНомера.УстановитьНомерДляПечатиВОбластьПечати(Область, "Номер", СтруктураДанных.Номер, СтруктураДанных.Ссылка, Неопределено, Истина);
		
		//ТабличныйДокумент.Вывести(Область);
		
		Для Каждого Стр Из ТабАванс Цикл
			
			ОбластьСтрока = Макет.ПолучитьОбласть("СтрАванс");
			ОбластьСтрока.Параметры.Заполнить(Стр);
			
			ВсеПараметрыМакетаНеЗаполнены = Истина;
			Для Каждого Параметр Из ОбластьСтрока.Параметры Цикл
				Если ЗначениеЗаполнено(Параметр) Тогда
					ВсеПараметрыМакетаНеЗаполнены = Ложь;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если ВсеПараметрыМакетаНеЗаполнены Тогда
				Продолжить;
			КонецЕсли;
		...
		Область = Макет.ПолучитьОбласть("ПодвалАванс"); 
		ГлавБух = ПечатьРегламентированныхФорм.ОтветственноеЛицоОрганизации(Ссылка.Организация, "ГлавныйБухгалтер", Ссылка.Дата);
		ПараметрыЗаполнения.Вставить("ГлБухгалтерИнициалыФамилия", Справочники.Сотрудники.ФамилияИО(ГлавБух, Истина)); 
		СтруктураДанных.Вставить("ИзрасходованоСуммаВРубляхПоКурсуАванса", ТаблицаРасчетов.Итог("СуммаРубПоКурсуАванса"));
		...
		Область = Макет.ПолучитьОбласть("ШапкаСведения");
		
		ТабличныйДокумент.Вывести(Область);
		
		Если ТабОстатокПерерасход.Количество() = 0 Тогда
			
			// две пустых строки
			ОбластьСтрока = Макет.ПолучитьОбласть("СтрСведения");
			ТабличныйДокумент.Вывести(ОбластьСтрока);
			ТабличныйДокумент.Вывести(ОбластьСтрока);
			
		Иначе
			
			Для Каждого Стр Из ТабОстатокПерерасход Цикл
				
				ОбластьСтрока = Макет.ПолучитьОбласть("СтрСведения");
				ОбластьСтрока.Параметры.Заполнить(Стр);
				РегистрыСведений.НастройкиПечатиПрефиксаНомера.УстановитьНомерДляПечатиВОбластьПечати(ОбластьСтрока,"НомерОрдера", Стр.НомерОрдера, Стр.Регистратор, Неопределено, Истина);
				ТабличныйДокумент.Вывести(ОбластьСтрока);
				
			КонецЦикла;
			
			// Добавим две пустые строки
		...
		Область = Макет.ПолучитьОбласть("ПодвалСведения");
...
Функция ПолучитьВыданныйОстатокПерерасход(Ссылка);
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
			|	ЖурналПроводокЕПСБУДвиженияССубконто.Регистратор КАК Регистратор,
			|	ЖурналПроводокЕПСБУДвиженияССубконто.КПСДт КАК КПС,
			|	ЖурналПроводокЕПСБУДвиженияССубконто.КЭКДт КАК КЭК,
			|	ЖурналПроводокЕПСБУДвиженияССубконто.Сумма КАК Сумма
			|ПОМЕСТИТЬ Движения
			|ИЗ
			|	РегистрБухгалтерии.ЖурналПроводокЕПСБУ.ДвиженияССубконто(, , ПОдстрока(СчетДт.Код, 1, 2) = ""18"", , ) КАК ЖурналПроводокЕПСБУДвиженияССубконто
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			...
			|ИЗ
			|	РегистрБухгалтерии.ЖурналПроводокЕПСБУ.ДвиженияССубконто(, , Подстрока(СчетКт.Код, 1, 2) = ""17"", , ) КАК ЖурналПроводокЕПСБУДвиженияССубконто
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	Движения.КПС КАК КПС,
			|	Движения.КЭК КАК КЭК,
			|	Движения.Сумма КАК Сумма
			|ПОМЕСТИТЬ ВТ_Сумма
			|ИЗ
			|	Документ.ОтчетОРасходахПодотчетногоЛица.ОстатокПерерасход КАК ОтчетОРасходахПодотчетногоЛицаОстатокПерерасход
			|		ЛЕВОЕ СОЕДИНЕНИЕ Движения КАК Движения
			|		ПО (ОтчетОРасходахПодотчетногоЛицаОстатокПерерасход.ДокументПерерасхода = Движения.Регистратор)
			|ГДЕ
			|	ОтчетОРасходахПодотчетногоЛицаОстатокПерерасход.Ссылка = &Ссылка
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			...
			|	Документ.ОтчетОРасходахПодотчетногоЛица.ОстатокПерерасход КАК ОтчетОРасходахПодотчетногоЛицаОстатокПерерасход
			|		ЛЕВОЕ СОЕДИНЕНИЕ Движения КАК Движения
			|		ПО ОтчетОРасходахПодотчетногоЛицаОстатокПерерасход.ДокументОстатка = Движения.Регистратор
			|ГДЕ
			|	ОтчетОРасходахПодотчетногоЛицаОстатокПерерасход.Ссылка = &Ссылка
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ВТ_Сумма.КПС КАК КПС,
			|	ВТ_Сумма.КЭК КАК КЭК,
			|	СУММА(ВТ_Сумма.Сумма) КАК Сумма
			|ИЗ
...
//Функция ПечатьОтчетаОРасходахПодотчетногоЛица_ОстатокПерерасход(МассивОбъектов, ОбъектыПечати) Экспорт
//	
//	ТабличныйДокумент = Новый ТабличныйДокумент;
//	ТабличныйДокумент.ИмяПараметровПечати = "ПФ_MXL_ОтчетОРасходахПодотчетногоЛица504520_ОстатокПерерасход";
//	
