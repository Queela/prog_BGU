...
Процедура АБК_ЗаполнитьПоРешениюОКомандировании(РешениеСсылка)
	...
	|		ИСТИНА КАК ЗапретИзменения
	|	) КАК ПрочиеРасходы,   
	|	РешениеОКомандировании.СпособыВыдачи.(
	|		Валюта КАК Валюта,
	...
	ВыборкаСпособыВыдачи = Выборка.СпособыВыдачи.Выбрать();
	Пока ВыборкаСпособыВыдачи.Следующий() Цикл

		НоваяСтрока = СпособыВыдачи.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаСпособыВыдачи);
	КонецЦикла;
...
Процедура АБК_ЗаполнитьПоРешениюОКомпенсацииРасходов(РешениеСсылка)
	...
	Если Выборка.Следующий() Тогда
		...
		#КонецВставки
		ТаблицаОбязательств = Документы.РешениеОКомпенсацииРасходовДляКрайнегоСевера.ТаблицаОбязательствПоРешениюОКомпенсации(
		Выборка.ДокументОснование);
		Для Каждого СтрокаОбязательства Из ТаблицаОбязательств Цикл
			НоваяСтрока = ДенежныеОбязательства.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаОбязательства);
...
Процедура АБК_ЗаполнитьПоЗаявкеОбоснованиюНаЗакупкуМалогоОбъема(ЗаявкаСсылка)
	...
	Если Выборка.Следующий() Тогда
		...
		#КонецВставки
		ТаблицаОбязательств = Документы.ЗаявкаОбоснованиеЗакупкиМалогоОбъема.ТаблицаДенежныхОбязательствПоЗаявке(
		Выборка.ДокументОснование);
		Для Каждого СтрокаОбязательства Из ТаблицаОбязательств Цикл
			НоваяСтрока = ДенежныеОбязательства.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаОбязательства);
...
Процедура АБК_ЗаполнитьПоЗаявкеОбоснованиюНаЗакупкуМалогоОбъемаЧерезПодотчетноеЛицо(ЗаявкаСсылка)
	...
	|			ИНАЧЕ ЗаявкаОбоснованиеЗакупкиМалогоОбъема.ТоварыРаботыУслуги.СпособОбеспечения
	...
	Если Выборка.Следующий() Тогда
		...
		#КонецВставки
		ТаблицаОбязательств = Документы.ЗаявкаОбоснованиеЗакупкиМалогоОбъемаЧерезПодотчетноеЛицо.ТаблицаДенежныхОбязательствПоЗаявке(
		Выборка.ДокументОснование);
		Для Каждого СтрокаОбязательства Из ТаблицаОбязательств Цикл
			НоваяСтрока = ДенежныеОбязательства.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаОбязательства);
