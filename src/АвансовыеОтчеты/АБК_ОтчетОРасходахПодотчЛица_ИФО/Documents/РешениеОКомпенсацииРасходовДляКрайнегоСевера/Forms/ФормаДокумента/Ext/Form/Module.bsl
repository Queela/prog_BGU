...
Процедура АБК_ОбновитьТаблицуФинансовоеОбеспечение()
	...
	Для Каждого ТЧ Из МассивТЧ Цикл  
		...
		Для Каждого СтрокаТЧ Из Объект[ТЧ] Цикл
			...
			КонецЕсли;
			#Удаление
			ЗаполнитьЗначенияСвойств(НоваяТаблицаФО.Добавить(), СтрокаТЧ);
			#КонецУдаления
			#Вставка     
	...
	КонецЦикла;
    #Удаление
	НоваяТаблицаФО.Свернуть("КФО, КПС, КЭК, КодЦели, КодМероприятия, ДопКлассификация", "Сумма");
	#КонецУдаления
	#Вставка     
	Если ЗначениеЗаполнено(ФинОбеспечениеИФО) Тогда 
		НоваяТаблицаФО.Свернуть("КФО, КПС, КЭК, КодЦели, КодМероприятия, ДопКлассификация," + ФинОбеспечениеИФО, "Сумма");
	Иначе 
		НоваяТаблицаФО.Свернуть("КФО, КПС, КЭК, КодЦели, КодМероприятия, ДопКлассификация", "Сумма");
	КонецЕсли;
	#КонецВставки
