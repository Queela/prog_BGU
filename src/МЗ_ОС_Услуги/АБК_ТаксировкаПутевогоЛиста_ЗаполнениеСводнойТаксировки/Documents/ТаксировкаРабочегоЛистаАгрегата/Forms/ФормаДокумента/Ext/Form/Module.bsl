...
Процедура АБК_ЗаполнитьСписываемыеМатериалыГСМНаСервере()
	...
	Иначе  
		...
		ТаблицаРасходаГСМ.Колонки.Добавить("Агрегат");
		Для Каждого Стр Из ТаблицаРасходаГСМ Цикл 
			Стр.Агрегат = Объект.Агрегат;	
		КонецЦикла;
	КонецЕсли;
	#КонецВставки
	...
	Для каждого ТекРасход Из Объект.ФактическийРасходГСМ Цикл 
		...
		Для каждого ТекРасход Из ТаблицаРасходаГСМ Цикл 
			...
			Если СписатьКоличество > 0 Тогда
				#Удаление
				НастройкиСписанияГСМПоУмолчанию = УчетГСМ.НастройкиСписанияГСМПоУмолчанию(Объект.Организация, Объект.Агрегат, СписатьВидГСМ);
				#КонецУдаления
				#Вставка     
				НастройкиСписанияГСМПоУмолчанию = УчетГСМ.НастройкиСписанияГСМПоУмолчанию(Объект.Организация, ТекРасход.Агрегат, СписатьВидГСМ);
				#КонецВставки
...
Функция ПолучитьТаблицуРасходаГСМ()
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТаксировкаРабочегоЛистаАгрегатаТаксировкиРабочихЛистов.ТаксировкаРабочегоЛиста КАК ТаксировкаРабочегоЛиста
	|ПОМЕСТИТЬ ВТ_Таксировки
	|ИЗ
	|	Документ.ТаксировкаРабочегоЛистаАгрегата.ТаксировкиРабочихЛистов КАК ТаксировкаРабочегоЛистаАгрегатаТаксировкиРабочихЛистов
	|ГДЕ
	|	ТаксировкаРабочегоЛистаАгрегатаТаксировкиРабочихЛистов.Ссылка = &Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаксировкаРабочегоЛистаАгрегатаФактическийРасходГСМ.ВидГСМ КАК ВидГСМ,
	|	ТаксировкаРабочегоЛистаАгрегатаФактическийРасходГСМ.ФактическийРасходГорючего КАК ФактическийРасходГорючего,
	|	ТаксировкаРабочегоЛистаАгрегатаФактическийРасходГСМ.НормативныйРасходГорючего КАК НормативныйРасходГорючего,
	|	ТаксировкаРабочегоЛистаАгрегатаФактическийРасходГСМ.Ссылка.Агрегат КАК Агрегат
	|ИЗ
	|	Документ.ТаксировкаРабочегоЛистаАгрегата.ФактическийРасходГСМ КАК ТаксировкаРабочегоЛистаАгрегатаФактическийРасходГСМ
	|ГДЕ
	|	ТаксировкаРабочегоЛистаАгрегатаФактическийРасходГСМ.Ссылка В
	|			(ВЫБРАТЬ
	|				ВТ_Таксировки.ТаксировкаРабочегоЛиста КАК ТаксировкаРабочегоЛиста
	|			ИЗ
