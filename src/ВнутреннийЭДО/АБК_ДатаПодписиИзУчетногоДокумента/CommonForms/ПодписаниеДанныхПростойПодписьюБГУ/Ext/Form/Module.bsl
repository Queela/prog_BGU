...
Процедура АБК_ДатаПодписанияВнутрЭДПодписатьВместо(Команда)
	...
	ПодписаниеУспешно = НЕ ТребуетсяВводПароля ИЛИ ПарольПроверенРанее ИЛИ ПарольПроверен();
	...
	Для Каждого ЭлементДанных Из ОписаниеДанных.НаборДанных Цикл
		...
		Если ЭтаФорма.ИспользоватьДатуДокумента Тогда 
				СвойстваПодписи.Вставить("ДатаПодписи", АБК_Дополнительные_Функции_ЭДО.ПолучитьДатуУчетногоДокумента(ЭлементДанных.УчетныйДокумент));
			Иначе
