...
&НаКлиенте
&Вместо("ПодписатьДанныеПослеОбработкиПередВыполнением")
Процедура АБК_ДатаПодписанияВнутрЭДПодписатьДанныеПослеОбработкиПередВыполнением(Результат, Контекст)
	...
	Для Каждого ЭлементДанных Из ОписаниеДанных.НаборДанных Цикл
		Если ЭтотОбъект.ИспользоватьДатуДокумента Тогда 
			ПараметрыВыполнения.Вставить("ДатаПодписи", АБК_ДополнительныеФункцииЭДО.ПолучитьДатуУчетногоДокумента(ЭлементДанных.Ссылка));
			//ПараметрыВыполнения.Вставить("БратьДатуДокумента",Истина);
		Иначе
			//ПараметрыВыполнения.Вставить("БратьДатуДокумента",Ложь);  
			ПараметрыВыполнения.Вставить("ДатаПодписи",ЭтотОбъект.ДатаПодписи);
		КонецЕсли;	 
	КонецЦикла;
	// Апогей БК
